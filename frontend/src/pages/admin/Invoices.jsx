...
const STATUS_TABS = [
  { value: '', key: 'tabAll' },
  { value: 'pending_review', key: 'tabPending' },
  { value: 'approved', key: 'tabApproved' },
  { value: 'rejected', key: 'tabRejected' },
]
...
function InvoiceRow({ invoice, products, onRefresh }) {
  ...
  async function mapItem(itemId, productId) {
    try {
      ...
      onRefresh()
    } catch (err) { toast.error(err.response?.data?.detail || t('admin.invoices.mapFail')) }
  }

  async function approve() {
    try {
      await api.post(`/invoices/${invoice.id}/approve`)
      toast.success(t('admin.invoices.approved'))
      onRefresh()
    } catch (err) { toast.error(err.response?.data?.detail || t('admin.invoices.approveFail')) }
  }

  async function reject() {
    try {
      await api.post(`/invoices/${invoice.id}/reject`)
      toast.success(t('admin.invoices.rejected'))
      onRefresh()
    } catch { toast.error(t('admin.invoices.rejectFail')) }
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            {formatDateTime(invoice.uploaded_at)} · {t('admin.invoices.items', { count: totalCount })}
            {invoice.status === 'pending_review' && totalCount > 0 && (
              <span className={`ml-2 font-medium ${allMapped ? 'text-green-600' : 'text-amber-600'}`}>
                {t('admin.invoices.mapped', { done: mappedCount, total: totalCount })}
              </span>
            )}
          </p>
        </div>
              ...
              return (
                ...
                >
                  ...
                  {invoice.status === 'pending_review' ? (
                    ...
                    >
                      <option value="">{t('admin.invoices.mapToProduct')}</option>
            ...
            <div className="border-t border-slate-100 px-4 py-3 flex items-center gap-3 bg-slate-50/50">
              {!allMapped && (
                <div className="flex items-center gap-1.5 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-1.5 mr-auto">
                  <AlertCircle size={12} />
                  {t('admin.invoices.mapRemaining', { count: totalCount - mappedCount })}
                </div>
              )}
              {confirming === 'reject' ? (
                <div className="flex items-center gap-2 ml-auto">
                  <span className="text-xs text-slate-600 font-medium">{t('admin.invoices.rejectConfirm')}</span>
                  <button onClick={reject} className="text-xs font-bold text-red-600 hover:text-red-800 px-2.5 py-1.5 rounded-lg bg-red-50 border border-red-200 transition-all duration-150 cursor-pointer">{t('admin.invoices.rejectYes')}</button>
                  <button onClick={() => setConfirming(null)} className="text-xs font-medium text-slate-500 hover:text-slate-700 px-2.5 py-1.5 rounded-lg bg-slate-100 transition-all duration-150 cursor-pointer">{t('admin.invoices.cancel')}</button>
                </div>
              ) : (
...
export default function Invoices() {
  ...
  ]).catch(() => toast.error(t('admin.invoices.loadFail')))

  useEffect(() => { load() }, [])

  async function handleFile(file) {
    if (!file || !file.name.endsWith('.pdf')) return toast.error(t('admin.invoices.onlyPdf'))
    const fd = new FormData()
    fd.append('file', file)
    setUploading(true)
    try {
      await api.post('/invoices/upload', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      toast.success(t('admin.invoices.uploaded'))
      load()
    } catch (err) { toast.error(err.response?.data?.detail || t('admin.invoices.uploadFail')) }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={t('admin.invoices.total', { count: invoices.length })}
        action={
          <label className={`cursor-pointer ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
            <Button as="span" variant="primary" icon={Upload} loading={uploading}>
              {uploading ? t('admin.invoices.uploading') : t('admin.invoices.uploadPdf')}
            </Button>
      ...
      {filtered.length === 0 && invoices.length === 0 ? (
        ...
        >
          ...
          <p className="font-semibold text-slate-700 mb-1">{dragging ? t('admin.invoices.dropNow') : t('admin.invoices.dropHere')}</p>
          <p className="text-sm text-slate-400">{t('admin.invoices.orBrowse')}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.length === 0 ? (
            <EmptyState icon={FileText} title={t('admin.invoices.noInvoices')} description={t('admin.invoices.noInvoices')} />
          ) : filtered.map(inv => (
